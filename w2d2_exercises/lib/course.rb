...
class Course
  ...
  def initialize(name = nil, department = nil, credits = nil, days = nil, time = nil)
    @name = name
  ...
  def conflicts_with?(second_course)
    return false if @time != second_course.time
    days_overlap?(second_course.days)
  end

  def days_overlap?(class2_days)
    class1_days = days
    overlap = false
    i = 0
    j = 0

    while i < class1_days.length
      while j < class2_days.length
        return true if class1_days[i] == class2_days[j]
        j += 1
      end
      i += 1
    end
